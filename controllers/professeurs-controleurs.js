...
const supprimerProfesseur = async (req, res, next) => {
  ...
  let cours;
  try {
    cours = await Cours.find({ professeur: professeur });
...
const ajouterCours = async (req, res, next) => {
  ...
  try {
    ...
    const erreur = new HttpErreur(
      "Ajout d'un cours à un professeur échoué",
      500
    );
    return next(erreur);
  }
  res.status(201).json({ message: "Le cours a été ajouté avec succès." });
};
