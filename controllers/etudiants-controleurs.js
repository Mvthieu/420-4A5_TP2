...
const ajouterEtudiant = async (req, res, next) => {
  ...
  const supprimerEtudiant = async (req, res, next) => {
    ...
    try{
        ...
        let cours = etudiant.cours;
        cours.forEach(unCours => {
            unCours.etudiants.pull(etudiantId);
            unCours.etudiants.save();
        });
  ...
  const inscrireEtudiant = async (req, res, next) => {
    ...
    try {
      ...
      return next(new HttpErreur("Inscription d'un étudiant à un cours échoué", 500));
    }
    ...
    try {
      ...
      const erreur = new HttpErreur("Inscription d'un étudiant à un cours échoué", 500);
      return next(erreur);
    }
